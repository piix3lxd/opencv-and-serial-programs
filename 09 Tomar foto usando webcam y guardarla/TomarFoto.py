...
# Función para capturar una foto con temporización
ejecutar_solo_una_vez = True

def capturar_con_temporizacion():
    global ejecutar_solo_una_vez
    if ejecutar_solo_una_vez == True:
        messagebox.showwarning("Advertencia", "Se están guardando fotogramas en un ciclo infinito, no te olvides de cerrar el programa o tu disco duro colapsará")
...
# SpinBox para que el usuario seleccione los segundos en los que se tomará la imagen automáticamente
...
BCapturar = tk.Button(ventana, text="Tomar foto única", command=Capturar)
BCapturar.place(x=500, y=305, width=100, height=23)

# Botón para capturar una foto temporizada
...
# Cuadro de imagen gris donde se reproducirá la cámara web
