...
def leer_datos_continuamente():
    while True:
        datos = SerialPort1.read_all().decode()
        if datos:
            TextRecibidos.insert(tk.END, datos)
            TextRecibidos.see(tk.END)  # Desplazar hacia abajo para mostrar los datos más recientes
        ventana.update_idletasks()  # Actualizar la interfaz gráfica sin bloquearla
...
def enviar_datos(mensaje):
    SerialPort1.write(mensaje.encode() + b"\r")
    ventana.after(500, leer_datos)  # Espera 500 milisegundos antes de leer los datos
...
# Iniciar el hilo para leer datos continuamente
thread_leer_datos = threading.Thread(target=leer_datos_continuamente, daemon=True)
thread_leer_datos.start()
